...
import { giftCardProvider } from '../giftCards/providers';
import type { PurchaseGiftCardOutput } from '../giftCards/providers';
import * as giftCardOrders from '../giftCards/giftCardOrders.service';
import type { Donation, Family, Prisma } from '@prisma/client';
...
export interface FulfillmentResult {
  ...
  outcome: 'completed' | 'manual_review' | 'already_completed' | 'noop_in_progress';
}

/** Encripta code/pin antes de persistir a resposta do provider — nunca texto puro em nenhuma coluna. */
function redactPurchase(purchase: PurchaseGiftCardOutput): Prisma.InputJsonValue {
  return {
...
async function createGiftCardFromPurchase(
  ...
  purchase: PurchaseGiftCardOutput,
  donationId: string,
  familyId: string,
): Promise<string> {
...
export async function fulfillPaidDonation(
  ...
  const order = await giftCardOrders.createProcessingOrder({
    ...
    externalProvider: giftCardProvider.name,
    idempotencyKey,
  ...
  try {
    purchase = await giftCardProvider.purchaseGiftCard({
      provider: donation.provider,
      amount: donation.amount,
      currency: 'BRL',
      donationId,
  ...
  // T2: a MESMA claim atômica de sempre — só roda porque o provider já entregou um código real.
  ...
  try {
    result = await prisma.$transaction(async (tx) => {
      ...
      const giftCardId =
        purchase.existingGiftCardId ?? (await createGiftCardFromPurchase(tx, purchase, donationId, family.id));

      const updatedDonation = await tx.donation.update({
        where: { id: donationId },
        data: { status: 'completed', giftCardId, completedAt: new Date() },
      });
      // payment já foi marcado 'paid' no início desta função (independente do resultado da emissão).
    ...
    await giftCardOrders.markOrderManualReview(order.id, 'fulfillment_transaction_error', redactPurchase(purchase));
    return finishAsManualReview(inFlight, family, 'fulfillment_transaction_error', actorUserId, order.id);
  }

  if (result.lostRace) {
    await giftCardOrders.markOrderManualReview(order.id, 'family_fed_by_another_donation', redactPurchase(purchase));
    return finishAsManualReview(inFlight, family, 'family_fed_by_another_donation', actorUserId, order.id);
  }

  await giftCardOrders.markOrderIssued(order.id, {
    externalOrderId: purchase.externalOrderId,
    rawResponseMetadata: redactPurchase(purchase),
  });
  ...
  await createAuditLog({
    ...
    metadata: { donationId, externalProvider: giftCardProvider.name },
  });
